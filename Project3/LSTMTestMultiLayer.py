    ...
    def forward(self, input, hx = None): 
        ...
        for t in range(seq_size):
            ...
            for tp in range(self.num_layers):
                ...
                c_tp = c_t[tp,:,:].t()
                # print("---------------------")
                # print((self.weight_ih[tp][i] @ x_t).shape)
                # print((self.bias_ih[tp][i].unsqueeze(0).t()).shape)
                # print((self.weight_hh[tp][i]).shape)
                # print(h_tp.shape)
                # print((self.weight_hh[tp][i] @ h_tp).shape)
                ...
                f_t = torch.sigmoid(self.weight_ih[tp][f] @ x_t + self.bias_ih[tp][f].unsqueeze(0).t() + self.weight_hh[tp][f] @ h_tp + self.bias_hh[tp][t].unsqueeze(0).t())
