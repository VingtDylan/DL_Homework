...
def load():
    ...
    LME_train_Aluminium, LME_validation_Aluminium = LME_train["Aluminium"], LME_validation["Aluminium"]
    LME_3M_train_Aluminium, LME_3M_validation_Aluminium = LME_3M_train["Aluminium"], LME_3M_validation["Aluminium"]
    # features merge
    train_data_label = pd.merge(LME_train_Aluminium,LME_3M_train_Aluminium, how = 'inner', on = 'date')
    validation_data_label =  pd.merge(LME_validation_Aluminium,LME_3M_validation_Aluminium, how = 'inner', on = 'date')
    #  more featues
    ...
    LME_train_Aluminium_Label_1d = LME_Label_train_1d["Aluminium"]
    LME_validation_Aluminium_1d = LME_Label_Validation["Aluminium" + "1d"]
    # merge data and label
    train_data_label = pd.merge(train_data_label,LME_train_Aluminium_Label_1d, how = 'inner', on = 'date')
    validation_data_label =  pd.merge(validation_data_label,LME_validation_Aluminium_1d, how = 'inner', on = 'date')
...
def main():
    ...
    args.batch_size  = 16
    # 加载数据集并切分
    Aluminium_train_data_label_1d, Aluminium_test_data_label_1d = load()
    sequence = args.sequence_length
    trainx, trainy = split_data_label(sequence = sequence,  data_label = Aluminium_train_data_label_1d)
    testx, testy = split_data_label(sequence = sequence,  data_label = Aluminium_test_data_label_1d)
